...
/** Orchestrates deterministic scan preparation and model perception. */
export class ScanPipeline {
  ...
  async run(input: ScanInput): Promise<ScanResult> {
    ...
    const artifacts = this.createArtifacts(frames, input); this.emit(scanId, 'extracting', 55, 'Scan artifacts prepared')
  ...
  private validate(input: ScanInput): void { if (!input.environmentId) throw this.error('INVALID_ENVIRONMENT', 'environmentId is required'); if (!input.source?.id) throw this.error('INVALID_SOURCE', 'source.id is required'); if (!input.media.uri) throw this.error('INVALID_MEDIA', 'media.uri is required'); if (!input.media.mimeType) throw this.error('INVALID_MEDIA', 'media.mimeType is required'); if (input.media.kind === 'image' && input.media.durationMs !== undefined) throw this.error('INVALID_MEDIA', 'image media cannot declare durationMs') }
  private sample(input: ScanInput): ScanFrame[] { const maxFrames = Math.max(1, input.options?.maxFrames ?? 24); const interval = Math.max(1, input.options?.sampleIntervalMs ?? 2000); const duration = input.media.durationMs ?? 0; const count = input.media.kind === 'image' ? 1 : Math.min(maxFrames, Math.max(1, Math.ceil(duration / interval))); return Array.from({ length: count }, (_, index) => ({ frameId: this.id('frame'), timestampMs: input.media.kind === 'image' ? 0 : Math.min(index * interval, duration), uri: input.media.uri })) }
