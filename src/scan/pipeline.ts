...
export class ScanPipeline {
 ...
 private createArtifacts(frames:ScanFrame[],input:ScanInput):ScanArtifact[]{const a=frames.map(f=>({artifactId:this.id('artifact'),frameId:f.frameId,kind:'frame' as const,uri:f.uri}));if(input.options?.preserveAudio&&input.media.kind==='video')a.push({artifactId:this.id('artifact'),kind:'audio',uri:input.media.uri});a.push({artifactId:this.id('artifact'),kind:'metadata',uri:input.media.uri});return a}
