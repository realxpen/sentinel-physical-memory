...
export class AskBuildingService {
  ...
  private buildContext(memory: EnvironmentalMemory, state: EnvironmentalState, question: string): AskContextScope {
    ...
    const stateObjects = snapshot.objects
    ...
    const context = [
      ...
      ...orderedStates.map((item) => {
        ...
        return `- STATE v${item.version} ${item.id} captured=${item.capturedAt} current=${item.id === memory.environment.currentStateId} | ${item.summary} | objects=${historySnapshot?.objects.length ?? 0} conditions=${historySnapshot?.conditions.length ?? 0} issues=${historySnapshot?.issues.length ?? 0} relations=${historySnapshot?.relations.length ?? 0}`
      }),
      ...
      selectedDiff ? `SELECTED_STATE_DIFF ${selectedDiff.id}: ${selectedDiff.summary}\n${selectedDiff.changes.map((change) => `- ${change.type}: ${change.title} | ${change.description} | confidence=${change.confidence} | evidence=${change.evidenceIds.filter((id) => renderedEvidenceIds.has(id)).join(',')}`).join('\n')}` : 'NO_DIFF_INTO_SELECTED_STATE',
  ...
  private diffLine(diff: EnvironmentalDiff, renderedEvidenceIds: Set<string>): string {
    return `- DIFF ${diff.id} ${diff.fromStateId}->${diff.toStateId}: ${diff.summary}\n${diff.changes.slice(0, 24).map((change) => `  - ${change.type}: ${change.title} | ${change.description} | confidence=${change.confidence} | evidence=${change.evidenceIds.filter((id) => renderedEvidenceIds.has(id)).join(',')}`).join('\n')}`
  }
