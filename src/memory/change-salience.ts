...
const DURABLE_ARCHITECTURAL_ANCHOR = /\b(?:doorway|door|window|room|corridor|hallway|entryway|entrance|frame)\b/
...
export function isLowSalienceObjectChange(change: Change): boolean {
  ...
  if ((change.type === 'added' || change.type === 'uncertain') && DURABLE_ARCHITECTURAL_ANCHOR.test(text)) return true
