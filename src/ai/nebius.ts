...
export class NebiusNemotronAdapter implements ModelAdapter, ReasoningModelAdapter, ActionPlanningModelAdapter, VerificationModelAdapter {
    ...
    try {
      const response = await this.fetchImpl(`${this.baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          model: this.model,
          temperature: 0,
          ...(role === 'perception' ? { max_tokens: 2200 } : role === 'temporal-verification' ? { max_tokens: 320 } : {}),
  ...
  private systemPrompt(role: ModelInferenceRequest['role']): string {
    return [
      ...
      'People require stricter visual grounding than ordinary objects. Emit category="person" only for an unmistakably visible real human being in the supplied current frame evidence. Never infer a person from a distant or ambiguous silhouette, reflection, poster/photo, signage, mannequin-like shape, foliage, furniture, shadows, or outdoor background detail.',
      'Every person object MUST include a tight boundingBox and a description that explicitly names at least two directly visible human cue groups: head/face, torso/body/shoulders, or limbs such as arms/hands/legs/feet. Use high confidence only when those cues make the human identity visually unmistakable. If a possible person is small, blurry, occluded, distant, or ambiguous, omit it.',
      'Do not infer or describe identity, demographics, emotion, intent, or other personal attributes. SENTINEL may remember a clearly observed person only as a grounded transient scene object after an independent confirmation pass.',
