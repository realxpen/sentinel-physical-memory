...
import { ModelAdapterError, type ArtifactContent, type ArtifactResolver, type ModelAdapter, type ModelInferenceRequest } from './model'
...
export class NebiusNemotronAdapter implements ModelAdapter {
  ...
  constructor(options: NebiusAdapterOptions) {
    ...
    this.apiKey = options.apiKey; this.baseUrl = (options.baseUrl ?? DEFAULT_BASE_URL).replace(/\/$/, '')
    this.model = options.model ?? DEFAULT_MODEL; this.fetchImpl = options.fetchImpl ?? fetch
    this.artifactResolver = options.artifactResolver; this.timeoutMs = options.timeoutMs ?? 60_000
  }
    ...
    try {
      const response = await this.fetchImpl(`${this.baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({ model: this.model, temperature: 0, messages: [
          { role: 'system', content: this.systemPrompt(request.role) }, { role: 'user', content },
        ] }), signal: controller.signal,
      })
      if (!response.ok) {
        const body = await response.text().catch(() => '')
        throw new ModelAdapterError({ code: 'NEBIUS_HTTP_ERROR', message: `Nebius inference failed (${response.status}): ${body.slice(0, 500)}`, status: response.status, retryable: response.status === 429 || response.status >= 500 })
      }
      const payload = await response.json() as ChatCompletionResponse
      return this.parsePerceptionResult(this.extractText(payload), request.artifacts)
    } catch (error) {
  ...
  private parsePerceptionResult(text: string, artifacts: ScanArtifact[]): PerceptionResult {
    let value: unknown
    try { value = JSON.parse(extractJson(text)) } catch { throw new ModelAdapterError({ code: 'INVALID_MODEL_JSON', message: 'Nemotron returned invalid JSON', retryable: false }) }
    try {
      const result = validatePerception(value)
      if (result.sourceId === 'unknown' && artifacts[0]) result.sourceId = artifacts[0].artifactId
      return result
    } catch (error) {
      if (error instanceof PerceptionValidationError) throw new ModelAdapterError({ code: error.code, message: error.message, retryable: false })
      throw error
...
function extractJson(text: string): string {
  ...
  const start = text.indexOf('{'); const end = text.lastIndexOf('}'); return start >= 0 && end > start ? text.slice(start, end + 1) : text.trim()
}
