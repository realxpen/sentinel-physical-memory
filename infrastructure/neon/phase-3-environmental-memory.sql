...
revoke all on schema sentinel_private from public;
grant usage on schema sentinel_private to sentinel_app;
...
revoke all on all tables in schema sentinel_private from public, sentinel_app;

create or replace function sentinel_private.sentinel_get_environmental_memory(p_environment_id text)
returns jsonb
language sql
stable
security definer
...
create or replace function sentinel_private.sentinel_save_environmental_memory(p_memory jsonb)
returns void
language plpgsql
security definer
set search_path = ''
...
revoke all on function sentinel_private.sentinel_get_environmental_memory(text) from public;
revoke all on function sentinel_private.sentinel_save_environmental_memory(jsonb) from public;
grant execute on function sentinel_private.sentinel_get_environmental_memory(text) to sentinel_app;
grant execute on function sentinel_private.sentinel_save_environmental_memory(jsonb) to sentinel_app;
