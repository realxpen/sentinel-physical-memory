...
try {
  ...
  expect(main.includes('current-memory-section'), 'Action Planner must coexist with the canonical current-object Memory surface')
  expect(main.includes('Create action plan ↗'), 'object inspector must expose contextual planning for actionable objects')
  expect(css.includes('.spatial-plan-button'), 'contextual object planning action styling is missing')
