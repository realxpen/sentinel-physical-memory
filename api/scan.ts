...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    ...
    if (input.media.kind === 'video') return res.status(422).json({ error: 'VIDEO_FRAME_EXTRACTION_REQUIRED', message: 'Video scanning is part of the contract, but live video inference requires server-side frame extraction before it is enabled.' })
    ...
    return res.status(200).json({ scanId: result.scanId, environmentId: result.environmentId, sourceId: result.source.id, completedAt: result.completedAt, frames: result.frames, observations: result.observations })
  } catch (error) {
...
function parseScanInput(value: unknown): ScanInput {
  ...
  return { environmentId, source: { id: sourceId, environmentId, modality: kind, uri, capturedAt, durationMs: optionalNumber(media.durationMs) }, media: { kind, uri, mimeType, durationMs: optionalNumber(media.durationMs), sizeBytes: optionalNumber(media.sizeBytes) }, options: { maxFrames: 1, sampleIntervalMs: 2000, preserveAudio: false } }
}
